...
===============================================*/
    public function searchTask() {
        ...
        $tasks = DB::table('tasks')
            ...
            $join->on('users.id', '=', 'tasks.user_id')->orOn('users.id', '=', 'tasks.assigned_by');
        })->where('users.name','LIKE','%'.$value.'%')->orWhere('tasks.task', 'LIKE', '%' . $value . '%')->orWhere('tasks.task_title', 'LIKE', '%' . $value . '%')
            ->select('tasks.*')
            ->distinct()
            ->get();

        return view('task.search', compact('value', 'tasks','users')  ) ;
    }
